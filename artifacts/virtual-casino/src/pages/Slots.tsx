...
import { Cherry, Diamond, Crown, Star, Gem, Bell, Coins, Info } from "lucide-react";
...
type SymbolDef = {
  ...
  payout2: number; // multiplier for 2 of a kind (only some symbols)
};

// Reel weights determine probabilities. Higher weight = more common.
const SYMBOLS: SymbolDef[] = [
  { id: "cherry", Icon: Cherry, color: "text-rose-400", weight: 30, payout3: 5, payout2: 1 },
  { id: "bell", Icon: Bell, color: "text-amber-400", weight: 22, payout3: 8, payout2: 0 },
  { id: "star", Icon: Star, color: "text-yellow-300", weight: 16, payout3: 15, payout2: 0 },
  { id: "gem", Icon: Gem, color: "text-cyan-400", weight: 10, payout3: 25, payout2: 0 },
  { id: "diamond", Icon: Diamond, color: "text-violet-400", weight: 6, payout3: 50, payout2: 0 },
  { id: "crown", Icon: Crown, color: "text-primary", weight: 3, payout3: 100, payout2: 0 },
];
...
function Reel({ finalSymbol, spinning, delay }: ReelProps) {
  ...
  return (
    <div className="relative w-24 h-24 sm:w-32 sm:h-32 overflow-hidden rounded-xl bg-gradient-to-b from-background via-card to-background border-2 border-primary/30 shadow-[inset_0_4px_24px_rgba(0,0,0,0.6)]">
      <div
...
export default function Slots() {
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-8">
      ...
      {/* Machine */}
      <div className="casino-card p-6 sm:p-10 relative overflow-hidden">
        <div className="absolute inset-x-0 top-0 h-px bg-gradient-to-r from-transparent via-primary/60 to-transparent" />
        <div className="flex justify-center gap-3 sm:gap-4 mb-8">
          {reels.map((sym, i) => (
            <Reel key={i} finalSymbol={sym} spinning={spinning} delay={i * 0.25} />
          ))}
        </div>
        ...
        <div className="text-center mb-6 h-12 flex items-center justify-center">
          {lastWin !== null && lastWin > 0 ? (
            <motion.div
              ...
              className="font-serif text-3xl casino-gradient-text"
            >
              + {lastWin} chips
            </motion.div>
          ) : lastWin === 0 && !spinning ? (
